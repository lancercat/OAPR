...
class prototyper_gen3_stand(gen3_object_to_feat_abstract):
    ...
    def forward_stub(this,normprotos, rot=0,engine=None):
        ...
        if (this.force_proto_shape is not None and pimage.shape[-1] != this.force_proto_shape):
            pimage = trnf.interpolate(pimage, this.force_proto_shape, mode="bilinear");
        if (rot > 0):
    ...
    def forward_debug(this, normprotos, rot=0, use_sp=True):
        ...
        allproto = trnf.normalize(allproto, dim=-1, eps=0.0009);
        # allprotoe,allatte=this.forward_debug_err(normprotos,rot,use_sp);
        return allproto.contiguous(),allatt
    def forward_debug_err(this, normprotos, rot=0, use_sp=True):
        if (len(normprotos) <= this.capacity):
            if (use_sp):
                spproto, _ = this.sp();
                p, a = this.forward_stub(normprotos, rot, engine=this.proto_engine_debug);
...
class prototyper_gen3_masked_stand(prototyper_gen3_stand):
    def proto_engine(this,clips):
        features,_ = this.backbone(clips)
        if(this.detached_ga):
            A = this.cam([f.detach() for f in features]);
        else:
            A =this.cam(features);
    ...
    def proto_engine_debug(this,clips):
        ...
        if (this.detached_ga):
            ...
            A = this.cam(features)        # A=torch.ones_like(A);
        out_emb = (A * features[-1]).sum(-1).sum(-1) / A.sum(-1).sum(-1);
        return out_emb,A;
...
class prototyper_gen3p_stand(prototyper_gen3_stand):
    ...
    def proto_engine_debug(this,clips):
        features = this.backbone(clips)
        A,b = this.cam(features,debug=True);
        # A=torch.ones_like(A);
        N,C,H,W=features[-1].shape;
        out_emb=(A.unsqueeze(2)*features[-1].reshape(N,A.shape[1],-1,W,H)).sum(-1).sum(-1)/A.sum(-1).sum(-1).unsqueeze(-1);
...
class prototyper_gen3peT_stand(prototyper_gen3pe_stand):
    def proto_engine(this,clips):
        features_ffn,features_att = this.backbone(clips)
        # detaching is now up to the GA itself.
        A = this.cam(features_att)
        # A=torch.ones_like(A);
        N,C,H,W=features_ffn[-1].shape;
        out_emb=(A.unsqueeze(2)*features_ffn[-1].reshape(N,1,-1,W,H)).sum(-1).sum(-1)/A.sum(-1).sum(-1).unsqueeze(-1);
        return  out_emb;
