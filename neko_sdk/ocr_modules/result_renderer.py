...
def dye(ids,srcdst,color):
    for i in ids:
        srcdst[i]=color;
    return srcdst;
def get_unpadded(im,length,kar=False,h=64,padvalue=0):
    try:
...
def render_words(dict,seenchs,im,gt_word,pred_words,flag=0,padvalue=0):
    try:
        Nep
    ...
    try:
        ...
        im1=get_unpadded(im,lgtp*32,padvalue=padvalue);
    ...
    if(len(pred_words)):
        ...
        return fin,[0]
def render_word(dict,seenchs,im,gt_word,pred_word,flag=0,padvalue=0):
    if(pred_word is not None):
        fin,neds=render_words(dict,seenchs,im,gt_word,[pred_word],flag,padvalue);
    else:
        fin,neds=render_words(dict,seenchs,im,gt_word,[],flag,padvalue);
    return fin,neds[0];
