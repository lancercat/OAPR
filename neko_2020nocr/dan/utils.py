...
def decode_prob_static(net_out,length,tdict):
    ...
    for i in range(0, length.shape[0]):
        try:
            current_idx_list = net_out[sta: sta + length[i]].topk(1)[1][:, 0].tolist();
            sta+=length[i];
        except:
            print(current_idx_list,sta,length);
...
def decode_k_static(net_out,length,k,tdict):
    ...
    for i in range(0, length.shape[0]):
        current_idx_list = net_out[int(length[:i].sum()): int(length[:i].sum() + length[i])].topk(k)[1][:,
                           0].tolist()
        current_text = ''.join(
            [tdict[_ - 1] if _ > 0 and _ <= len(tdict) else tUNKREP for _ in current_idx_list])
        out.append(current_text)
    return out
class cha_encdec():
